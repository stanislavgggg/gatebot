...
# Спрашивать язык на первом /start отдельным экраном.
# По умолчанию выключено: язык определяется автоматически по клиенту
# Telegram, а сменить его можно кнопкой 🌐 на гейте и в хабе. Лишний
# экран перед гейтом стоит конверсии на холодном платном трафике.
ASK_LANG_ON_START = os.getenv("ASK_LANG_ON_START", "0").lower() in ("1", "true", "yes")

# ГЕО, которые сознательно работают БЕЗ гейта (подписка не требуется).
# Список через запятую, например: GATE_FREE_GEOS=en
#
# По умолчанию пусто — ГЕО без каналов считается ненастроенным и
# прячется. Иначе забытая переменная окружения молча превращалась бы
# в свободный доступ ко всем бонусам.
GATE_FREE_GEOS = {
    x for x in os.getenv("GATE_FREE_GEOS", "").replace(" ", "").lower().split(",") if x
}
...
class Geo:
    ...
    def is_ready(self) -> bool:
        """
        ГЕО готово к работе, если у него есть канал ЛИБО оно явно
        внесено в GATE_FREE_GEOS.

        Без каналов гейт пропускает всех подряд (missing пустой →
        mark_passed), то есть юзер получает доступ, ни на что не
        подписавшись. Это допустимо только как осознанное решение,
        а не как следствие незаполненной переменной окружения.
        """
        return bool(self.channels) or self.code in GATE_FREE_GEOS

    @property
    def is_gated(self) -> bool:
        """Требует ли ГЕО подписки на каналы."""
